...
// imageUrl が空のエントリのみ更新（auto-round の公開直後補完用。全件更新は週次 GitHub Actions が担当）
const asins = Object.keys(cache).filter(a => !cache[a].imageUrl);
if (asins.length === 0) {
    console.log('No ASINs in cache, nothing to update');
    return;
}

if (asins.length === 0) { console.log('No entries with empty imageUrl. Nothing to do.'); process.exit(0); }
